...
This lecture applies earlier concepts rather than introducing new algorithms.
\begin{itemize}[leftmargin=*]
  \item From \textbf{Unit 04 Lectures 01--02}, recall how metric choice depends on the cost of errors.
  \item From \textbf{Unit 03 Lecture 02}, recall leakage, delayed labels, and drift-aware evaluation.
  \item From \textbf{Unit 05 Lecture 01}, keep the responsible-AI checklist active while reading each domain case.
\end{itemize}
...
For each domain, answer the same five questions:
\begin{itemize}[leftmargin=*]
  \item What is the \textbf{task}? (classification, regression, anomaly detection, ranking, etc.)
  \item What \textbf{data} and \textbf{labels} are available?
  \item Which \textbf{metric} best reflects the real cost of errors?
  \item What is the main \textbf{risk}? (drift, bias, adversaries, safety, privacy)
  \item Which \textbf{monitoring signal} would tell us the system is going wrong?
\end{itemize}
...
\paragraph{Case-study snapshot.}
\begin{itemize}[leftmargin=*]
  \item \textbf{Task:} binary classification for card fraud; sometimes anomaly detection for unusual behavior.
  \item \textbf{Metric:} precision/recall and cost-aware evaluation because FP and FN have different business costs.
  \item \textbf{Main risk:} proxy bias and drift when fraud patterns or economic behavior change.
  \item \textbf{Monitoring signal:} chargeback rate, false-decline rate, and distribution changes in transaction features.
\end{itemize}
...
\paragraph{Case-study snapshot.}
\begin{itemize}[leftmargin=*]
  \item \textbf{Task:} phishing classification or network anomaly detection.
  \item \textbf{Metric:} high recall matters, but alert volume must stay manageable for analysts.
  \item \textbf{Main risk:} adversarial drift because attackers adapt intentionally.
  \item \textbf{Monitoring signal:} alert rate, analyst-confirmed incident rate, and sudden shifts in email/network feature patterns.
\end{itemize}
...
\paragraph{Case-study snapshot.}
\begin{itemize}[leftmargin=*]
  \item \textbf{Task:} risk prediction, imaging classification, or triage support.
  \item \textbf{Metric:} recall/sensitivity is often critical when missing a positive case can cause harm; calibration can also matter.
  \item \textbf{Main risk:} unsafe deployment under dataset shift across hospitals, devices, or populations.
  \item \textbf{Monitoring signal:} performance by site/population, missing-data patterns, and clinician override/escalation rates.
\end{itemize}
