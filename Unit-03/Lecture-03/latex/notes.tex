...
This lecture assumes two earlier Unit III ideas are already clear:
\begin{itemize}[leftmargin=*]
  \item from \textbf{Unit 03 Lecture 01}: features are design choices, and their scale/representation affects the model,
  \item from \textbf{Unit 03 Lecture 02}: preprocessing must be fit on the \textbf{training split only} to avoid leakage.
\end{itemize}
PCA is a feature-transformation method, so both ideas still apply.
...
\subsection{How to get eigenvalues and eigenvectors manually}
For PCA, you do \textbf{not} find eigenvalues directly from the raw data matrix.
You first compute the covariance matrix $C$, and then solve:
\[
  C v = \lambda v
\]
where:
\begin{itemize}[leftmargin=*]
  \item $\lambda$ is an eigenvalue, which tells how much variance that direction captures,
  \item $v$ is an eigenvector, which gives the direction of that component.
\end{itemize}

For a small matrix, the manual steps are:
\begin{enumerate}[leftmargin=*]
  \item Form $C$ from the centered data.
  \item Solve the characteristic equation:
  \[
    \det(C - \lambda I)=0
  \]
  to obtain the eigenvalues.
  \item For each eigenvalue $\lambda$, solve:
  \[
    (C-\lambda I)v=0
  \]
  to obtain the corresponding eigenvector.
  \item Normalize the eigenvector to unit length.
  \item Sort eigenvalues in decreasing order; the eigenvector of the largest eigenvalue is the first principal component.
\end{enumerate}
This is practical only for tiny examples such as $2 \times 2$ covariance matrices. Real software uses more stable matrix methods internally.

\subsection{Worked 2D example: manual eigenvalues and eigenvectors}
Suppose that after centering the data, the covariance matrix is:
\[
  C =
  \begin{bmatrix}
    3 & 1\\
...
\paragraph{Step 1: Find eigenvalues.}
Solve:
\[
  \det(C-\lambda I)=
  \det
...
\paragraph{Step 2: Find eigenvector for $\lambda_1 = 4$.}
\[
  C - 4I =
  \begin{bmatrix}
    -1 & 1\\
...
\paragraph{Step 3: Find eigenvector for $\lambda_2 = 2$.}
\[
  C - 2I =
  \begin{bmatrix}
    1 & 1\\
...
\paragraph{Interpretation for PCA.}
Because $\lambda_1 = 4$ is larger than $\lambda_2 = 2$, the first principal component is the direction:
\[
  \hat v_1 = \frac{1}{\sqrt{2}}
  \begin{bmatrix}
    1\\1
  \end{bmatrix}
\]
This is the diagonal direction. PCA keeps this direction first because it captures more variance than the perpendicular direction.
...
\section{LDA and ICA (conceptual preview)}
\subsection{LDA}
...
\paragraph{Why it is only a preview here.}
LDA makes much more sense after you have seen classification properly in \textbf{Unit 04 Lecture 02}. For now, remember only this anchor: PCA ignores labels, while LDA uses them.
...
\end{lstlisting}
The key rule is unchanged from the previous lecture: fit scaling and PCA on \textbf{training data only}, then reuse the learned transformation on validation/test data.
