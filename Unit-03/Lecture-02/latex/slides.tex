...
\begin{frame}{Before We Split: What Must Already Be Clear?}
  \begin{itemize}[<+->]
    \item From Lecture 01: know what the \textbf{features}, \textbf{labels}, and \textbf{task type} are.
    \item Choose the split strategy \emph{after} identifying the task: classification, regression, grouped data, or time series.
    \item Honest evaluation means the \textbf{test set stays untouched} until the end.
  \end{itemize}
\end{frame}
...
\begin{frame}{Worked Story: Churn Prediction}
  \begin{itemize}[<+->]
    \item Suppose 1{,}000 customers are available and only about 12\% churn.
    \item Split into train/test first, then use \textbf{stratified 5-fold CV} inside training.
    \item Example validation F1 scores: 0.71, 0.75, 0.73, 0.70, 0.76.
    \item Choose the model using the average CV score; report the test score only once at the end.
  \end{itemize}
\end{frame}
...
X_train, X_test, y_train, y_test = train_test_split(
  ...
  \end{lstlisting}
  \vspace{0.3em}
  \small Use \texttt{stratify=y} for classification tasks. For regression, omit it and choose a split strategy that matches the data source.
\end{frame}
