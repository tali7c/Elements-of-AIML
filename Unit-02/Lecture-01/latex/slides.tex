...
\begin{frame}{Entailment First: What Are We Trying to Show?}
  \begin{itemize}[<+->]
    \item Before converting anything to CNF, clarify the goal:
    \[
      KB \models \alpha
    \]
    means every model that satisfies $KB$ must also satisfy $\alpha$.
    \item Example:
    \[
      KB = \{p \rightarrow q,\; p\}, \qquad \alpha = q
    \]
    \item Intuition: CNF is a \textbf{format for automated reasoning}, not the meaning itself.
    \item First understand the question semantically; then choose a proof method.
  \end{itemize}
\end{frame}
...
\begin{frame}{After CNF: Entailment vs Derivation}
  \begin{itemize}[<+->]
    ...
    \item CNF helps mostly on the \textbf{proof side} when we use procedures such as resolution.
