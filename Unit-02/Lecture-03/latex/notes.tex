...
This lecture assumes two earlier routines are already familiar:
\begin{itemize}[leftmargin=*]
  \item from \textbf{Unit 02 Lecture 01}: entailment, unsatisfiability, and basic CNF conversion,
  \item from \textbf{Unit 02 Lecture 02}: predicates, quantifiers, and substitution.
\end{itemize}
If any of those ideas feel weak, quickly revisit the earlier notes first. Resolution is mechanical only after the prerequisite language is stable.
...
\subsection{Worked end-to-end example: ``Socrates is mortal''}
English statements:
\begin{itemize}[leftmargin=*]
...
\paragraph{Step 1: translate to FOL.}
\[
...
\paragraph{Step 2: set up refutation.}
To prove $KB \models Mortal(Socrates)$, add the negated query:
\[
  \lnot Mortal(Socrates)
\]

\paragraph{Step 3: convert to clause form.}
\begin{itemize}[leftmargin=*]
  \item $\forall x\; (Human(x) \rightarrow Mortal(x))$ becomes $\lnot Human(x) \lor Mortal(x)$.
  \item $Human(Socrates)$ is already an atomic clause.
  \item $\lnot Mortal(Socrates)$ is already an atomic clause.
\end{itemize}
Clause set:
\[
  C_1:(\lnot Human(x) \lor Mortal(x)),\quad
  C_2:Human(Socrates),\quad
  C_3:\lnot Mortal(Socrates)
\]

\paragraph{Step 4: resolve with unification.}
Resolve $C_1$ with $C_2$ using substitution $\{x/Socrates\}$:
\[
  C_4:Mortal(Socrates)
\]
...
\[
  \Box
\]
Because the empty clause is derived, the negated query is inconsistent with the KB, so the original query is entailed.
