...
\begin{frame}{Three Paradigms, Three Feedback Sources}
  \begin{itemize}[<+->]
    \item \textbf{Clustering}: no labels, no rewards; discover groups already present in the data.
    \item \textbf{Semi-supervised}: a few labels + many unlabeled examples; extend a supervised model carefully.
    \item \textbf{RL}: no fixed labels per row; the agent learns from delayed rewards during interaction.
  \end{itemize}
\end{frame}
...
\begin{frame}{Failure Mode: Wrong Pseudo-Labels}
  \begin{itemize}[<+->]
    \item Start with very few labeled examples from a rare class.
    \item A weak first model can assign confident but wrong pseudo-labels.
    \item If we retrain on those errors, the model becomes more biased, not less.
  \end{itemize}
\end{frame}
...
\begin{frame}{Reinforcement Learning (High Level)}
  \begin{itemize}[<+->]
    ...
    \item Feedback is delayed and noisy, so evaluation is harder than in supervised learning
  \end{itemize}
\end{frame}

\begin{frame}{Return and Discounting (Key RL Idea)}
  \begin{itemize}[<+->]
    \item We care about \textbf{total future reward}, not only the current reward.
    \item A discount factor $\gamma$ controls short-term vs long-term focus.
    \item High $\gamma$ values encourage planning for later rewards.
  \end{itemize}
\end{frame}
